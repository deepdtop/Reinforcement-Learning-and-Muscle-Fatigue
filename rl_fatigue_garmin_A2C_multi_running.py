...
def extract_fit_metrics(file_path_eval):
    """
    ...
    """
    filename = f'/Users/danterracina/Documents/FATIGUE AND RL/carreras_filtered/{file_path_eval}'
...
class FatigueEnv:
    ...
    def _get_val(self):
        ...
        efficiency = self.pre_efficency_specific[0] / 200 - self.pre_efficency_specific[1] / 400 - \
                     self.pre_efficency_specific[2] / 12 + self.pre_efficency_specific[3] / 20 \
                     + self.pre_efficency_specific[4] / 450 \
                     - 0.8 * self.current_fatigue / 100 - 5 * np.sqrt(
            (self.current_biomeca_levels[2] / 12 - self.pre_efficency_specific[2] / 12) ** 2 +
...
class A2CAgent:
    ...
    def compute_advantage(self, rewards, values, next_value, dones, gamma=0.99, lam=0.95):
        ...
        values_tensor = torch.tensor([v.item() if isinstance(v, torch.Tensor) else float(v) for v in values],
                                     dtype=torch.float32)
...
if __name__ == '__main__':
    models_folder = 'deep_rl_fatigue_model_multi_run'
    rewards_folder = 'deep_rl_fatigue_rewards_multi_run'
    num_episodes = 200
    plot_all = False
    batch_size = 64
    initial_fatigue = 5
    ...
    file_pattern = "./carreras_filtered/*.pkl"  # adjust the path and pattern as needed
    all_runs = glob.glob(file_pattern)
    if type_of == 'train':
        for idx_tr, run_evaluated in enumerate(all_runs):
            try:
                if idx_tr == len(all_runs) - 2: #keep 2 for test
                    break
                print(f'RUN EVALUATED: {run_evaluated}')
                data, columns_data = extract_fit_metrics(os.path.basename(run_evaluated))
                ...
                for e in range(num_episodes):
                    ...
                    if plot_all == True and (e == num_episodes % num_episodes or  e == int(
                            2 * num_episodes / 4)  or e == num_episodes - 1):
    ...
    if type_of == 'train':
        agent.save(f'{models_folder}/mod_')
    ...
    if type_of =='test':
        ...
        for idx_tr, run_evaluated in enumerate(all_runs[-2:]):
            try:
                ...
                state_size = env.state_dim
                action_size = (env.action_space.shape[0])
                agent_test = A2CAgent(state_size, action_size, scale_eval)
                agent_test.load(f'{models_folder}/mod_')
                print(f'action size {action_size}')
                ...
                for e in range(num_episodes):
                    t0 = datetime.datetime.now()
                    val, actions, fatigue, reward_l, state_evol = play_one_episode(agent_test, env, type_of)
                    dt = datetime.datetime.now() - t0
                    print(f"episode: {e + 1}/{num_episodes}, episode and value: {val:.2f}, duration: {dt} ")
                    efficiency_evol.append(val)
                    all_actions.append(actions)
                    all_fatigues.append(fatigue)
                    all_reward.append(reward_l)
                    if plot_all_test == True and (e == num_episodes % num_episodes or e == int(num_episodes / 4) or e == int(
                        ...
                        # State evolution
                        ax5 = fig.add_subplot(2, 4,
                                              4)  # ['Cadence', 'GCT', 'vertical_ratio', 'enhanced_speed', 'power']
                        ax5.plot(state_evol[:, 5], color='orange', linestyle=':', label='Cadence pred')
                        ax5.plot(state_evol[:, 0], color='orange', label='Cadence true')
                        ax5.plot(state_evol[:, 6], color='green', linestyle=':', label='GT pred')
                        ax5.plot(state_evol[:, 1], color='green', label='GT true')
